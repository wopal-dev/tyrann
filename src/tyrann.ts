...
export type TyrannConfig<OperationType extends Operation> =
  AxiosRequestConfig & {
...
export const tyrann = <Paths extends BasePaths, ApiType extends Api<Paths>>(
  ...
  ) => Promise<MapOperationToResponse<NonNullable<ApiPaths[PathType][Method]>>>;
  ...
  ) => Promise<MapOperationToResponse<NonNullable<ApiPaths[PathType]["post"]>>>;
  ...
  ) => Promise<MapOperationToResponse<NonNullable<ApiPaths[PathType]["put"]>>>;
  ...
  const fetch: FetchType = async (method, path, config) => {
    ...
    if (operation.pathParams) {
      ...
      const sanitizedParams = await operation.pathParams.validate(
        config?.pathParams!
      );
    ...
    if (operation.query) {
      ...
      const finalParams =
        "__isYupSchema__" in operation.query
          ? sanitizedParams
          : operation.query.transform(sanitizedParams);

      if (Object.keys(finalParams).length > 0) {
        finalUrl +=
          "?" + queryString.stringify(finalParams, config.queryOptions);
      }
    }
    ...
    const warnError = () => {
      console.warn(
        `${method} ${fullPath} received unexpected data with code ${
          response.status
    ...
    let schema: yup.Schema | undefined = responseDef;

    if (schema === undefined) {
      warnError();
      throw new Error(
        `${method} ${fullPath} response of status ${response.status} is not handled`
      );
    }
    ...
    try {
      ...
      warnError();
      throw new Error(
        `${method} ${fullPath} got invalid data: ${JSON.stringify(
          (e as any).errors,
          undefined,
          2
        )}`
  ...
  const post: PostType = (path, body, config) =>
    fetch("post", path, { data: body, ...config });
  const put: PutType = (path, body, config) =>
    fetch("put", path, { data: body, ...config });
  const del: MethodType<"delete"> = (path, config) =>
    fetch("delete", path, config);
